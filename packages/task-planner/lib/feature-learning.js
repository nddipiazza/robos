...
function build(record,plan,modules=[]){if(record.kind!=='feature')throw Error('Select a feature to create its e-learning.');if(!plan?.trim())throw Error('Draft and save the feature plan in Task Planner before creating e-learning.');const version=hash(plan),id='urn:robos:elearning:feature:'+record.id+':'+version.slice(0,16);
...
async function create(record,plan,graphRoot,{generate=require('../../robos-lib/course-editor-main').revise}={}){
 ...
 const run=(async()=>{
  ...
  const proposal=workspace.propose({mode:'refine',edits:[old?{op:'update',id,set}:{op:'add',node:course}],prompt:'Generate learner-focused lessons from the saved feature plan; retain source provenance in metadata.',requireEvidence:true});
