...
| **Dev Discussions** | Discord/Slack-like work-item & PR review discussion threads (Projects -> Features -> Tasks/PRs) with smart caching |
...
- **`testing`** (`robos.testing`): Behavior-Driven Development (BDD) Gherkin grammar (`robos:GherkinFeature`, `robos:GherkinBackground`, `robos:GherkinRule`, `robos:Scenario`, `robos:ScenarioOutline`, `robos:ExamplesTable`, `robos:StepDefinition`, `robos:DataTable`, `robos:DocString`), quality assurance test plans (`robos:TestPlan`), test suites (`robos:TestSuite`), execution records (`robos:TestExecutionRecord`), and 16+ major testing frameworks (`robos:TestingLibrary`).

**Upstream Schema Provenance & Agent Code Generation (`robos:refersFrom`)**:
All 98 W3C SHACL constraint shapes in RobOS are formally annotated with their upstream canonical schema basis (`robos:refersFrom` / `rdfs:isDefinedBy`), pointing to standards such as Schema.org, OASIS OSLC (Requirements Management, Quality Management, Change Management, Architecture Management), C4 Model, or Cucumber. When autonomous agents generate or extend KGraph schemas, they MUST base their output on this referred object and expand upon it with domain-specific SDLC properties.

**Source-Backed Graph Workspaces & Iterative Reviewed Imports (`ROBOS_GRAPH_ROOT`)**:
RobOS supports external, source-backed knowledge graph workspaces with two-phase reviewed import and refinement workflows:
- **ACID Graph Workspace & Crash Recovery** (`GraphWorkspace`): Managed via `packages/robos-graph/lib/graph-workspace.js`. Guarantees ACID multi-file writes across `.robos/kgraphs/<pkg>/package.jsonld` and `knowledge-graph.jsonld` with atomic hardlink locking (`write.lock`), write-ahead journal preflights (`pending-import.json`), and crash recovery (`kgraph recover`).
- **Two-Phase Review Cycle (`propose` → `apply`)**: Graph modifications (imports, agent brief refinements, migrations) generate hash-addressed proposals with 3-way conflict detection, property-level diffs, and validation gates before commit. Canonical graph files are never mutated directly without review.
- **Evidence-Backed Source Extractor** (`source-extractor.js`): Inspects local Git checkouts hermetically without executing code, importing modules, or running Git hooks. Attaches cryptographic `robos:evidence` (repository, relative path, line number, revision, and SHA-256) to all extracted nodes.
- **Built-in Source Declaration Shapes**: Includes 6 lightweight source declaration shapes (`SourceArtifact`, `CurriculumDefinition`, `AgentSkill`, `DataStore`, `BrokerDefinition`, `EnvironmentProfile`) representing source-level declarations before live runtime facts or deployment endpoints are known.
- **Source-Only Companion App Guard**: Desktop apps (`data-sources`, `kube-studio`, `remote-execution-studio`) honor `ROBOS_GRAPH_ROOT` in a source-only read mode, blocking unverified mutation probes and simulated endpoints.
- **Workspace CLI & MCP Tools**: Workspaces can be operated via CLI (`init`, `inspect`, `context`, `propose`, `apply`, `recover`) or the Model Context Protocol (`robos_sdlc_inspect`, `robos_sdlc_query`, `robos_sdlc_propose`, `robos_sdlc_apply`).
...
## Web App Launch Kit (`nextjs-vercel-web` template)

Every new public web app (`robos:FrontEndApp`, Next.js, Vercel) starts from
`packages/app-wizard/templates/nextjs-vercel-web`, generated by `packages/app-wizard/lib/generate-app.js`
(App Wizard → Front End App, or `node -e "require('./packages/app-wizard/lib/generate-app').generateNewApp({...}, {robosRoot: '.'})"`).
It is the production launch kit distilled from getemgigs.com. Agents building or reviewing a web app MUST keep
these in place (or record a reason in the app's KGraph node):

1. **Data & auth** — Neon Postgres via `DATABASE_URL` (PGlite locally/tests, no setup), idempotent `src/lib/schema.js`,
   email + password with server-side hashed sessions, account deletion that cascades user data.
2. **Abuse controls** — Postgres-backed rate limits, honeypot + form timing, disposable-email block, password rules,
   same-origin (CSRF) checks, CSP/HSTS headers, audit log. reCAPTCHA v3 stays wired but disabled until keys are set.
3. **Search & AI-agent discoverability** — `src/app/robots.js` (search engines, AI search/agents such as OAI-SearchBot,
   ChatGPT-User, Claude-SearchBot, ClaudeBot, PerplexityBot, and link-preview bots allowed on public pages; private
   paths disallowed), `sitemap.js` with dynamic public URLs, `public/llms.txt`, canonical + Open Graph/Twitter metadata,
   generated OG image and PNG icons, web manifest, schema.org JSON-LD for every public entity (e.g. `MusicEvent`,
   `Product`), `noindex` on private pages. All site facts live in `src/lib/site.js`.
4. **Mobile-first UI** — design at 375px first, bottom tab bar when signed in, 16px inputs, safe-area insets.
5. **Proof of work** — Cucumber + Playwright scenarios in `tests/e2e`; each actor is a separate recorded phone
   (CDP screencast with wall-clock timestamps, fake camera for QR flows); `npm run evidence` renders the Cucumber
   splash intro, step HUD and multi-phone timeline; `scripts/make-hero-gif.sh` makes the landing hero GIF.
   Run the suite against production (`npm run e2e:live`) before calling a feature done.
6. **KGraph** — the app's `robos:FrontEndApp` node (applications package) records `robos:appTemplate`,
   `robos:productionUrl`, `robos:hostingPlatform`, `robos:databaseEngine`, `robos:authStrategy`,
   `robos:abuseControls`, `robos:captchaProvider`, `robos:crawlerPolicy`, `robos:sitemapUrl`, `robos:llmsTxtUrl`,
   `robos:structuredData` and `robos:e2eEvidence` (optional properties on `FrontEndAppShape`).

When a real app improves the kit (a new header, a better crawler rule, a new evidence feature), port it back into
the template, bump `TEMPLATE_VERSION` in `generate-app.js`, and keep `packages/robos-test/tests/app-wizard/generate-app.test.js` green.
