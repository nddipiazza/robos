...
app.whenReady().then(() => {
  ...
  if (_debugServer) _debugServer.startDebugServer(win, 19113);
...
function launchUnlockDialog() {
  ...
  // Try local dev path first, then VM install path
  const candidates = [
    path.resolve(__dirname, '..', 'pass-unlock'),
    '/usr/local/share/robos/pass-unlock',
  ];
  const unlockDir = candidates.find(d => fs.existsSync(path.join(d, 'main.js')));
  if (!unlockDir) return; // pass-unlock not installed, skip silently

  const electronCandidates = [
    path.join(unlockDir, 'node_modules', 'electron', 'dist', 'electron'),
    path.join(unlockDir, 'node_modules', '.bin', 'electron'),
  ...
  try {
    spawn(electronBin, [unlockDir, '--no-sandbox', '--disable-gpu', '--disable-dev-shm-usage'],
