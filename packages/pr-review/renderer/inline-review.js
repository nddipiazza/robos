...
(()=>{
 ...
 window.openInlineReview=async({ctx,path,line,side,row})=>{
  ...
  panel.innerHTML='<header><strong></strong><button type="button" data-close aria-label="Close inline review">×</button></header><div role="tablist"><button role="tab" data-tab="comment" aria-selected="true">PR comment</button><button role="tab" data-tab="fix" aria-selected="false">AI fix</button></div><section data-pane="comment" role="tabpanel"><div data-comments></div><label>Comment<textarea placeholder="Leave feedback on this line…" rows="3"></textarea></label><button type="button" data-post>Post comment</button></section><section data-pane="fix" role="tabpanel" hidden><p>Run a focused fix on this PR’s branch using the saved task plan. Follow the new session in Task Runner.</p><robos-ai-textarea show-submit="false" show-commands="false" show-agent="false" min-height="100" placeholder="Describe what the agent should fix on this line…"></robos-ai-textarea><button type="button" data-fix>Configure &amp; run fix…</button></section><p role="status" aria-live="polite"></p>';
  panel.querySelector('strong').textContent=`${path}:${line} · ${side==='LEFT'?'Old':'New'} version`;row.after(panel);
  const status=panel.querySelector('[role=status]'),comment=panel.querySelector('textarea'),fix=panel.querySelector('robos-ai-textarea'),post=panel.querySelector('[data-post]'),run=panel.querySelector('[data-fix]');
  ...
  let busy=false;const update=()=>{post.disabled=busy||!comment.value.trim();run.disabled=busy||!fix.value?.trim();};
  ...
  run.onclick=async()=>{if(run.disabled)return;try{valid();await window.openTaskRunnerLaunch({mode:'implement',call:async(action,input)=>{
    valid();
