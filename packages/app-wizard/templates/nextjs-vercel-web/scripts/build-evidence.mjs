#!/usr/bin/env node
// RobOS evidence video builder for __APP_NAME__ Cucumber E2E runs.
//
...
//   2. a 1920x1080 composite where every actor's real phone recording of the site is
...
const BRAND = process.env.EVIDENCE_BRAND || __APP_NAME_JS__;
const BRAND_HOST = process.env.EVIDENCE_HOST || '__DOMAIN__';
...
function splashHtml(tl) {
  ...
  <div class="head"><span class="cat">${esc(BRAND.toUpperCase())} — BDD CUCUMBER E2E · ROBOS PROOF-OF-WORK</span>
...
function chromeHtml(tl, idx, t0, T, slots) {
  ...
  const phones = tl.actors
    .map((a, i) => {
      ...
      <div class="label ${active ? 'active' : ''}" style="left:${s.x}px;top:${s.y - 46}px;width:${PHONE_W}px">📱 ${esc(a.name)}${a.label ? ` · ${esc(a.label)}` : ''}${active ? ' <b>● ACTING</b>' : ''}</div>`;
    })
    .join('');
  ...
  <div class="left"><div class="brand">RobOS E2E Evidence · ${esc(BRAND_HOST)}</div><div class="scen">${esc(tl.scenario)}</div><ol>${items}</ol></div>
...
async function main() {
  ...
  const reel = path.join(EVIDENCE, '__APP_SLUG__-e2e-evidence-reel.mp4');
