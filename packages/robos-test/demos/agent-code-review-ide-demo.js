...
const SCRIPT = [
  ...
  {
    narration: "We select PR #42 ('feat: PET-105 mTLS Rabies Verification') from the real repository pull request queue.",
    target: '.pr-card[data-number="12"]',
    action: "click",
    callout: "Select PR #42: PET-105 mTLS Rabies Certificate Verification",
    js: `(() => {
  ...
  },
  {
    ...
    js: `(() => {
      ...
      setTimeout(() => {
        ...
        if (textarea) textarea.value = "Approved! Verified mTLS client implementation in IntelliJ & VS Code. OpenAPI contract, 14/14 Pact tests, and Knowledge Graph branch confirmed.";
  ...
  },
  {
    ...
    action: "hover",
    callout: "PR #42 Merged & Synchronized — Full Review Lifecycle Complete",
    minHold: 4500,
  },
];
...
2. **Pull Request Triage**: Selecting PR #42 (\`feat: PET-105 mTLS Rabies Verification\`).
...
`;
    ...
    try {
      if (fs.existsSync(videoPath)) {
        ...
        execSync(`ffmpeg -y -ss 00:00:44 -i "${videoPath}" -vframes 1 "${PERSIST_DIR}/agent-review-dual-merge_frame.png"`, { stdio: "ignore" });
