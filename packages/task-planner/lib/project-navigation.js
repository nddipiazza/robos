...
function navigation(project) {
  ...
  const product=kind==='project'?{id:project.id,name:project.name}:project.product || (name ? {id:workspace,name:name==='MVP'?'MVP':name.split(/[-_]/).map(s=>s.charAt(0).toUpperCase()+s.slice(1)).join(' ')} : null);
