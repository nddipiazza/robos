...
function resolveIconPath(iconRef) {
  ...
  return iconRef; // keep theme name as-is
}
...
function syncRegistryFromDesktopFiles() {
  ...
  const desktopFiles = fs.readdirSync(DESKTOP_DIR)
    .filter(f => f.endsWith('.desktop') && f.startsWith('robos-') || f.endsWith('.desktop'));
  ...
  for (const filename of desktopFiles) {
    ...
    if (!parsed.name.toLowerCase().includes('robos')) continue; // only RobOS apps
    ...
    if (!reg.icons[appId]) {
      ...
      // Keep user customizations; just refresh desktopFile path and label
...
// ── Update Icon= in a .desktop file ─────────────────────────────────────────
function updateDesktopIcon(desktopFile, newIconPath) {
  try {
    let contents = fs.readFileSync(desktopFile, 'utf8');
    contents = contents.replace(/^Icon=.*$/m, `Icon=${newIconPath}`);
    fs.writeFileSync(desktopFile, contents, 'utf8');
    return true;
  } catch { return false; }
}
...
function buildMergedIcons() {
  ...
  // Seed with builtin apps (provides iconSvg + metadata for all known apps)
  ...
  // Overlay with any filesystem registry data (user customisations / installed paths)
  for (const [appId, entry] of Object.entries(reg.icons)) {
    if (merged[appId]) {
      // Keep builtin iconSvg as fallback; overlay installed path and desktop file
...
ipcMain.handle('ri-push-icons', async (event) => {
  ...
  for (let i = 0; i < BUILTIN_APPS.length; i++) {
    ...
    // Write 64×64 SVG to the app's icon file
    ...
    // Update Icon= in the .desktop file
    if (fs.existsSync(desktopFile)) {
      ...
      result.desktopUpdated = false; // no installed .desktop yet
    }

    results.push(result);
    event.sender.send('ri-push-progress', result);
    // Yield briefly so the renderer can repaint between items
...
ipcMain.handle('ri-sync-desktop-files', () => {
  ...
  for (const [appId, entry] of Object.entries(reg.icons)) {
    ...
    const ok = updateDesktopIcon(entry.desktopFile, entry.iconPath);
    results.push({ appId, ok });
  }
...
app.whenReady().then(() => {
  ...
  // Bootstrap registry on first launch
