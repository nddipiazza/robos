...
function proposePlan(root, input, fetchIssue = readIssue) {
  ...
  for (const item of plan.items) {
    nodes.push({ ...base, '@id': issueId(item.url), '@type': [`robos:${item.type}`], 'dcterms:title': `#${item.issue.number} ${item.issue.title}`, 'dcterms:description': item.issue.body, 'robos:status': item.issue.state, 'robos:url': item.url, 'robos:inProject': { '@id': projectId }, ...(item.type==='Bug'?{'robos:severity':'unknown'}:{}), ...(item.parent ? { 'robos:inFeature': { '@id': issueId(item.parent) } } : {}), ...(item.agentPersonaId ? { 'robos:assignedAgent': { '@id': item.agentPersonaId } } : {}), ...(item.assignedRole ? { 'robos:assignedRole': item.assignedRole } : {}), ...(item.dependsOn?.length ? { 'robos:dependsOn': item.dependsOn.map(url => ({ '@id': issueId(url) })), 'robos:relationshipEvidence':item.dependsOn.map(url=>({predicate:'robos:dependsOn',target:issueId(url),evidence,note:'Ordered delivery dependency recorded in the project plan.'})) } : {}) });
  }
