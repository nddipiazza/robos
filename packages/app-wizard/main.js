...
const fs = require('fs');
const os = require('os');
const { generateNewApp } = require('./lib/generate-app');
...
ipcMain.handle('app-wizard:generate-new-app', async (_, spec) => {
  // Web apps are generated from templates/nextjs-vercel-web (the getemgigs.com launch kit):
  // auth, Neon/PGlite, abuse controls, reCAPTCHA scaffold, SEO + AI-agent discoverability,
  // Cucumber E2E with RobOS evidence videos. See lib/generate-app.js.
  try {
    return generateNewApp(spec, { robosRoot: getRobosRoot(), homeDir: os.homedir() });
  } catch (err) {
    return { error: err.message };
  }
});
