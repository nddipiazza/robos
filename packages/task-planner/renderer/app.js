...
async function loadAgentPersonas() {
  ...
  if (!agentPersonas.length) {
    agentPersonas = [
      ...
      { id: 'urn:robos:agent:non-headless-dev', slug: 'non-headless-dev', role: 'Non-Headless Developer', icon: '💻', title: 'RobOS Non-Headless Ephemeral GUI Developer Agent', category: 'GUI & IDE Automation', executionMode: 'ephemeral-gui', systemPrompt: 'You are an autonomous Non-Headless Developer in RobOS.', developmentGuidance: 'Use ephemeral storage & Xvfb (:99), mapped IDE via IPC, step debugger, Chrome DevTools MCP, and token-saving IDE refactorings.' },
      { id: 'urn:robos:agent:human-agent-copilot', slug: 'human-agent-copilot', role: 'Human + Agent Desktop Co-Pilot', icon: '👥', title: 'RobOS Human + Agent Desktop Co-Pilot Agent', category: 'Desktop Pairing', executionMode: 'desktop-session', systemPrompt: 'You are an interactive Human + Agent Desktop Co-Pilot in RobOS.', developmentGuidance: 'Collaborate in the user active desktop session (:0), share IDE and terminal, step-debug together, and run IDE refactorings live.' },
    ];
  }
}
