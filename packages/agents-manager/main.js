...
// Try to load copilot-lib for --check-provider mode
let copilotLib = null;
try { copilotLib = require('/usr/local/share/robos/robos-copilot-lib'); } catch {}
...
app.whenReady().then(async () => {
  if (checkProviderMode && copilotLib) {
    const activeId = copilotLib.getActiveProviderId();
    const status = await copilotLib.checkProviderStatus(activeId);
