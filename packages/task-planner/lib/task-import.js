...
async function search(server,{query='',state='open'},projects,gh=core.gh){
 ...
 const args=['issue','list','--repo',repo,'--state',state,'--limit','100','--json','number,title,body,state,url,labels,updatedAt'];
...
async function importIssues(server,{projectId,numbers},projects,dir,gh=core.gh,saveState=core.save){
 ...
 for(const number of new Set(numbers)){
  try{
   const issue=await gh(['issue','view',String(number),'--repo',repo,'--json','number,title,body,state,url,labels,updatedAt,assignees']);
   ...
   const feature=issue.labels.some(l=>/^(feature|epic|type:feature|type:epic)$/i.test(l.name));
   const saved=saveProject(dir,{id,name:issue.title,kind:feature?'feature':'task',product:target.product,workTaskUrl:issue.url,serverId:server.id,prompt:`${issue.url}\n\n${issue.body||''}`,tasks:[{title:issue.title,body:issue.body||'',labels:issue.labels.map(l=>l.name),ticketKey:`#${issue.number}`,ticketUrl:issue.url,ticketStatus:issue.state}],sourceUpdatedAt:issue.updatedAt});
