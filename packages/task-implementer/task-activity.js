...
function activity(urls){const result={};for(const url of (urls||[]).slice(0,200)){if(!/^https:\/\/github\.com\//.test(url))continue;core.identity(url);const state=core.read(url);result[url]={...progress(state),...require('../robos-agent-client/work-task/plan-approval').readiness(state),planApproved:require('../robos-agent-client/work-task/plan-approval').approved(state)};}return result;}
