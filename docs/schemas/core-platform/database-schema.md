...
- **Provenance & Alignment**: When autonomous agents generate, expand, or validate instances of `robos:DatabaseSchema`, they MUST adhere to and base their output on this referred schema object, extending it with RobOS SDLC properties.
...
|---|---|---|---|---|
| **`dcterms:title`** | Title / Display Name | `1..*` | `xsd:string` | Database Schema must have a title. |
| **`robos:schemaName`** | Database Schema Name | `1..*` | `xsd:string` | Database Schema must specify schema name. |
| **`robos:database`** | Parent Database | `1..*` | `URI (robos:Database | robos:NoSQLDatabase)` | Database Schema must link to parent database. |
