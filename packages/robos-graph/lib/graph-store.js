...
`;
      ...
      // Bundle the canonical shared player; generated apps do not maintain a renderer fork.
      const shared = path.resolve(__dirname, '../../robos-lib');
      for (const [source, name] of [
        [path.join(shared, 'elearning-player.js'), 'elearning-player.js'],
        [path.join(shared, 'node_modules/marked/lib/marked.umd.js'), 'marked.js'],
        [path.join(shared, 'node_modules/dompurify/dist/purify.min.js'), 'purify.js'],
      ]) fs.copyFileSync(source, path.join(finalDir, 'renderer', name));
      fs.writeFileSync(path.join(finalDir, 'renderer/index.html'), '<!doctype html><html><head><meta charset="utf-8"><title>RobOS e-learning</title><link rel="stylesheet" href="style.css"></head><body><p id="status" role="status"></p><robos-elearning-player id="player"></robos-elearning-player><script src="marked.js"></script><script src="purify.js"></script><script src="elearning-player.js"></script><script src="app.js"></script></body></html>');
      fs.writeFileSync(path.join(finalDir, 'renderer/style.css'), 'body{margin:0;padding:24px 32px;background:#0d1117;color:#dce8f1;font:15px system-ui}');
      fs.writeFileSync(path.join(finalDir, 'renderer/app.js'), "'use strict';window.addEventListener('DOMContentLoaded',async()=>{try{const data=await window.robosELearning.getCourseData();document.getElementById('player').course=data.course;}catch(e){document.getElementById('status').textContent=e.message;}});");
