...
class RobosWorld extends World {
  constructor(options) {
    super(options);
    this.actors = new Map();
    this.vars = new Map(); // scenario scratch values shared between steps
  ...
  async actor(name) {
    ...
    const a = { name, browser: b, cameraFile, context, page, email: null, password: null, label: null, frames: [], cdp: null };
