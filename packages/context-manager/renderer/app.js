...
function fileIcon(rel) {
  ...
  if (rel.includes('package.json') || rel.includes('pom.xml') || rel.includes('go.mod')) return '📦';
...
function renderMdFull(md) {
  let html = md
    // Escape HTML in non-code regions handled below
...
async function generateKnowledgeGraph(src) {
  ...
  _attachSpinner(btn);
  statusEl.textContent = 'Asking Copilot to analyze context and generate Mermaid diagram…';
  statusEl.classList.remove('hidden');
  ...
  statusEl.classList.add('hidden');
  containerEl.innerHTML = '<div id="mermaid-render"></div>' +
    `<details style="margin-top:12px"><summary style="font-size:11px;color:#8b949e;cursor:pointer">View Mermaid source</summary><div class="graph-raw">${esc(r.mermaid)}</div></details>`;

  // Render mermaid
  try {
    const m = window._mermaid;
    if (m) {
      const { svg } = await m.render('kg-' + Date.now(), r.mermaid);
      document.getElementById('mermaid-render').innerHTML = svg;
    } else {
      document.getElementById('mermaid-render').innerHTML =
        '<span class="list-empty" style="color:#d29922">Mermaid not loaded yet. Try again in a moment.</span>';
    }
  } catch (e) {
    document.getElementById('mermaid-render').innerHTML =
      `<span class="list-empty" style="color:#f85149">Render error: ${esc(e.message)}</span>`;
  }
}
  ...
  function renderProjectsList(q) {
    ...
    available.forEach(proj => {
      ...
      item.innerHTML = `
        ...
        <div class="gh-proj-meta">${esc(nameWithOwner)}${proj.localPath ? ' &bull; ' + esc(proj.localPath) : ''}</div>
      `;
      item.onclick = () => {
        selectedRepo = proj;
