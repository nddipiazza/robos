...
function register({app,ipcMain,dialog},screen){
  ...
  ipcMain.handle('work-task-review',wrap(async({prUrl})=>{const s=current();if(!s.prs.some(p=>p.url===prUrl))throw Error('PR is not linked to this task');const ctx=await review.context(prUrl);reviews.set(prUrl,{...ctx,passed:false,inspected:false});return{pr:ctx.pr,diff:ctx.diff,reviewPolicy:require('./review-policy').get(review.prIdentity(prUrl).repo),questions:ctx.questions.map(({label,options})=>({label,options}))};}));
