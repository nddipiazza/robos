...
export async function userForToken(token) {
  ...
  const row = await one(
    `SELECT u.id, u.email, u.display_name, u.created_at,
            b.id AS band_id, b.name AS band_name, b.genre AS band_genre, b.hometown AS band_hometown,
            b.bio AS band_bio, b.reputation AS band_reputation
       FROM sessions s
  ...
  return {
    ...
    band: row.band_id
      ? {
          ...
          hometown: row.band_hometown,
          bio: row.band_bio,
          reputation: row.band_reputation,
        }
      : null,
  };
}
