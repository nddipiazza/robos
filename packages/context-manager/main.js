...
// ── Shared copilot helper via robos-copilot-lib ───────────────────────────────
const copilot = require('/usr/local/share/robos/robos-copilot-lib');
const runCopilotPrompt = (prompt) => copilot.ask(prompt, { source: 'context-manager' });
...
ipcMain.handle('read-special-files', (_, src) => {
  ...
  const copilot = tryRead('.github/copilot-instructions.md') || tryRead('copilot-instructions.md');
  return { ok: true, agents, copilot };
});
