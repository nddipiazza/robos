---
name: create-robos-app
description: Create and scaffold a new RobOS Electron desktop app with full registrations and deployment.
---
...
### 2. Register in `packages/robos-icons/builtin-apps.js` and `packages/robos-icons/builtin-apps-browser.js`
...
### 3. Update AGENTS.md App Suite table
...
- Verify the app is registered in `packages/robos-icons/builtin-apps.js`
- Verify the app is deployed to `/usr/local/share/robos/<app-id>/` on the VM with world-readable permissions
- Verify the .desktop file is installed at `/usr/share/applications/<app-id>.desktop` on the VM
