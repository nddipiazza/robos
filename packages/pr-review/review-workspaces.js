...
function ideOptions(state,prUrl){if(!(state.prs||[]).some(p=>p.url===prUrl))throw Error('PR is not linked to the current task.');const projects=ides.projects();return ides.choicesFor((state.launchConfig?.repositories||[]).map(url=>projects.find(p=>ides.normalize(p.url)===ides.normalize(url))||{url}));}
async function open(state,prUrl,ideId,reviewedHead){const review=await pages(state),page=review.pages.find(p=>p.url===prUrl);if(!page)throw Error('PR is not linked to the current task.');if(reviewedHead&&page.headRefOid!==reviewedHead)throw Error('The PR changed. Reload its page before opening the IDE workspace.');page.ide=ides.selectIDE(ideOptions(state,prUrl),ideId);const workspace=await prepare(state,page,core.command,review.pages);require('../robos-lib/ide-trust').trustReviewWorkspace(workspace.target,workspace.reviewRoot,workspace.ide);return {...await ides.launch(workspace.ide.id,workspace.target),repositories:workspace.repositories};}
...
module.exports={ideOptions,assertPrerequisites,assertMergeOrder,repositoryPaths,orderPages,pages,prepare,writeWorkspace,open};
