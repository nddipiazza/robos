...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 async function load(url){const ticket=++generation;panel.innerHTML='<header><h2>Tasks in this feature</h2><button class="btn btn-outline" id="refresh-feature-tasks">Refresh</button></header><p role="status">Loading tasks from the task server…</p><robos-list-sort for="linked-feature-task-rows" original="Feature order" label="feature tasks"></robos-list-sort><div class="feature-task-rows"></div>';
 panel.querySelector('button').onclick=()=>load(url);document.getElementById('planner-tab-tasks').textContent='Tasks (…)';
 try{const result=await window.robos.featureTasks(url);if(ticket!==generation||currentUrl!==url)return;if(!result.ok)throw Error(result.error);loaded=result.tasks;document.getElementById('planner-tab-tasks').textContent=`Tasks (${loaded.length})`;panel.querySelector('[role=status]').textContent=loaded.length?'Linked tasks from GitHub, in their feature order.':'No child tasks are linked to this ticket on the task server.';
