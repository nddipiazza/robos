...
function register({app,ipcMain,dialog},screen){
  ...
  ipcMain.handle('work-task-state',wrap(()=>{if(!url)return null;const s=current();const dir=core.folder(url);let output='';for(const mode of ['plan','implement']){const file=path.join(dir,`${mode}-output.txt`);if(fs.existsSync(file))output+=fs.readFileSync(file,'utf8').slice(-40000);}let events=[];const eventsFile=path.join(dir,'events.jsonl');if(fs.existsSync(eventsFile))events=fs.readFileSync(eventsFile,'utf8').split('\n').filter(Boolean).flatMap(line=>{try{return [JSON.parse(line)];}catch{return [];}});return{...s,screen,output,events};}));
  ...
  ipcMain.handle('work-task-save',wrap(({plan,workspace})=>{current();return core.save(url,{plan,workspace,approvedPlanHash:null,phase:'plan-review'});}));
  ipcMain.handle('work-task-approve-plan',wrap(({plan,workspace})=>{current();if(!plan?.trim())throw Error('A plan is required');const old=current();if(old.workerPid)throw Error('Wait for the running agent before approving a plan.');return core.save(url,{plan,workspace:workspace||old.workspace,approvedPlanHash:core.planHash(plan),phase:'plan-approved'});}));
  ipcMain.handle('work-task-agent',wrap(async input=>{current();return core.startWorker(url,input.mode,input,process.execPath);}));
  ...
  ipcMain.handle('work-task-merge',wrap(async({prUrl,notes,reviewed})=>{current();const ctx=reviews.get(prUrl);if(!ctx?.passed||!ctx.inspected||reviewed!==true)throw Error('Complete the knowledge check and review the changes and evidence before approving.');const merged=await review.approveAndMerge(prUrl,ctx.pr.headRefOid,notes);core.save(url,{phase:'merged',merged,reviewCertificate:{prUrl,head:ctx.pr.headRefOid,approvedAt:new Date().toISOString(),notes}});return merged;}));
}
