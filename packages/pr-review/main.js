...
for(const [channel,method] of [['pr-review-source','source'],['pr-inline-comment','inlineComment'],['pr-inline-comments','inlineComments']])ipcMain.handle(channel,async(_,input)=>{try{return await realTheater[method](input);}catch(e){return {ok:false,error:e.message};}});
...
ipcMain.handle('pr-review-evidence',async(_,input)=>{try{return await realTheater.evidence(input,require('../robos-agent-client/work-task/electron').currentURL());}catch(e){return {ok:false,error:e.message};}});
ipcMain.handle('pr-review-revision',async(_,input)=>{try{
 const {repo,number}=require('../robos-agent-client/work-task/review').prIdentity(input.url);
 const pr=await require('../robos-agent-client/work-task/core').gh(['api',`repos/${repo}/pulls/${number}`]);
 return {ok:true,head:pr.head.sha,body:pr.body||'',title:pr.title,state:pr.state};
}catch(e){return {ok:false,error:e.message};}});
