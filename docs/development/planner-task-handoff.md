...
The new-task Submit action explains that submission approves the reviewed plan
using the signed-in GitHub identity. It validates required-reviewer rules before
syncing, sends the native Task issue type, and retains the exact plan and its
approval in the runner session. The local Planner identity survives submission,
