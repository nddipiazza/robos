...
describe('RobOS eLearning Website Generator & GitHub Pages Publishing', () => {
  ...
  it('1. Generates standalone interactive website for robos-crpg with Jekyll frontmatter for GitHub Pages', () => {
    const targetHtmlPath = path.join(rootRepo, 'docs', 'elearning', 'robos-crpg-engine.html');

    const result = store.generateELearningWebsite({
      courseId: 'robos-crpg',
      outputFilePath: targetHtmlPath,
      permalink: '/elearning/robos-crpg-engine',
    });

    assert.strictEqual(result.ok, true, 'Result ok must be true');
    assert.strictEqual(result.filePath, targetHtmlPath);
    assert.strictEqual(result.permalink, '/elearning/robos-crpg-engine');
    ...
    assert.ok(fs.existsSync(targetHtmlPath), 'docs/elearning/robos-crpg-engine.html must exist on disk');

    // Verify directory index file exists for /elearning/robos-crpg-engine/ routing
    const indexFilePath = path.join(rootRepo, 'docs', 'elearning', 'robos-crpg-engine', 'index.html');
    ...
    assert.ok(content.includes('permalink: /elearning/robos-crpg-engine'), 'Must specify exact GitHub Pages permalink');
    ...
    assert.ok(content.includes('cRPG Realm Project'), 'Must include breadcrumb link to project');

    // 3. Verify all 5 curriculum modules are present
    assert.ok(content.includes('D&amp;D 5e Ruleset') || content.includes('D&D 5e Ruleset'), 'Module 1 must be present');
    assert.ok(content.includes('Godot 4.3 Engine Runtime'), 'Module 2 must be present');
    assert.ok(content.includes('Open-Source Asset Harvesting'), 'Module 3 must be present');
    assert.ok(content.includes('Autonomous Infinity AI Agent'), 'Module 4 must be present');
    assert.ok(content.includes('Knowledge Graph-First Game Architecture'), 'Module 5 must be present');
  ...
  it('4. Documentation pages link to the interactive web edition', () => {
    ...
    assert.ok(docContent.includes('/elearning/robos-crpg-engine'), 'elearning-masterclass.md must link to web edition');

    const realmDoc = path.join(rootRepo, 'docs', 'projects', 'crpg-realm.md');
    const realmContent = fs.readFileSync(realmDoc, 'utf8');
    assert.ok(realmContent.includes('/elearning/robos-crpg-engine'), 'crpg-realm.md must link to web edition');
  });
});
