...
- **`core-platform`** (`robos.core`): System architecture, C4 topology, microservices, relational and NoSQL databases (`robos:Database`, `robos:NoSQLDatabase`), Kafka message brokers (`robos:MessageBroker`), Model Context Protocol servers (`robos:MCPServer`), AI context sources (`robos:ContextSource`), and distributed build systems (`robos:RemoteExecutionCluster`, `robos:BuildSystem`).
