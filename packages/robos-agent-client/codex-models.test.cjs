...
test('CLI discovery initializes and pages through visible model list without starting an agent',async()=>{
 const cli=fixture(r=>r.method==='initialize'?{id:r.id,result:{}}:r.method==='model/list'?{id:r.id,result:r.params.cursor?{data:[{model:'two',displayName:'Second'},{model:'hidden',hidden:true}],nextCursor:null}:{data:[{model:'one',displayName:'First',isDefault:true,defaultReasoningEffort:'low',supportedReasoningEfforts:[{reasoningEffort:'low',description:'Fast'},{reasoningEffort:'high',description:'Thorough'}]}],nextCursor:'next'}}:null);
 const models=await listModels('codex',cli);assert.deepEqual(models.map(m=>m.id),['one','two']);assert(models[0].isDefault);assert.deepEqual(models[0].reasoningEfforts.map(e=>e.id),['low','high']);assert.equal(models[0].defaultReasoningEffort,'low');assert.deepEqual(cli.sent.map(m=>m.method),['initialize','initialized','model/list','model/list']);assert(cli.killed());
});
