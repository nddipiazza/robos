...
async function renderProjectIDE(project) {
 ...
 const open=panel.querySelector('[data-open]');const availability=()=>{open.disabled=!checkout.value;open.title=checkout.value?'Open this checkout with its configured project roots':'Add a local checkout to open it in an IDE';};checkout.onchange=availability;ide.onchange=availability;availability();
 ...
 action('[data-open]',async()=>{const result=await window.robosOpenIDE({load:()=>gp.workspaceIDEOptions(project.id),launch:ideId=>gp.openWorkspace({id:project.id,workspace:checkout.value,ideId})});status.textContent=result?'Opened '+(result.opened||[result]).map(r=>r.ide).join(', '):'';});
}
