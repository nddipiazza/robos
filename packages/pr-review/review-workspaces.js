...
async function prepare(state,page,run=core.command,allPages=[page]){
 ...
 for(const rootInfo of discovered.length?discovered:[{relativePath:'.'}])workspaceRoots.push({name:repo.name+(rootInfo.relativePath==='.'?'':'/'+rootInfo.relativePath),path:projectRoots.resolve(repo.path,rootInfo.relativePath)});}
