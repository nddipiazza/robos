...
async function render(icons) {
  ...
  // Only load file images for entries that have a custom iconPath but no builtin SVG
  const imgData = await Promise.all(
    entries.map(e =>
      !e.iconSvg && e.iconPath && e.iconPath.startsWith('/')
        ? window.api.readImage(e.iconPath)
  ...
  grid.innerHTML = entries.map((e, i) => {
    ...
    // Lucide builtin SVG is the primary display icon.
    // fileImg (user-customised or explicitly installed path) overrides it.
    let imgHtml;
    if (e.iconSvg && !fileImg) {
      // Builtin Lucide SVG — inject inline (same-origin, controlled content)
      imgHtml = `<div class="card-svg">${e.iconSvg}</div>`;
    } else if (fileImg) {
      // User has a custom / installed icon file that overrides the builtin
      imgHtml = `<img class="card-img" src="${esc(fileImg)}" alt="${esc(e.label)}">`;
    } else {
      imgHtml = `<span class="card-placeholder">🖼</span>`;
    }

    const iconLabel = fileImg ? shortPath(e.iconPath) : (e.iconName ? `${e.iconName} (Lucide)` : '(none)');
    const hasCustom = !!fileImg;

    return `
      <div class="icon-card${hasCustom ? ' has-custom' : ''}" data-appid="${esc(e.appId)}" title="${esc(e.desc || e.label)}">
...
async function changeIcon(appId) {
  const res = await window.api.updateIcon(appId);
  if (!res.ok) return;
...
document.getElementById('btn-push-icons').addEventListener('click', async () => {
  ...
  const res = await window.api.pushIcons();
...
(function() {
  ...
  const _entry = _apps.find(a => a.appId === 'robos-icons');
  if (_entry) {
