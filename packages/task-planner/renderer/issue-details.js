...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 window.refreshPlannerUX=function(...args){refresh(...args);const record=selectedPlannerRecord(),meta=record?.issueMetadata;const kind=record?.kind||meta?.type||'Plan';const number=meta?.number||plannerSession?.issue?.number;
 ...
 for(const [label,url] of [['View ticket on task server',meta.url],['Parent epic',meta.parentUrl],...meta.dependencies.map(url=>['Dependency',url])])if(url){const b=document.createElement('button');b.className='btn btn-outline';b.textContent=label;b.title=url;b.onclick=()=>window.robos.openUrl(url);panel.append(b);}}
 window.renderPlannerChatLinks?.(panel,record);
 const title=document.createElement('h2');title.textContent='Git repositories';const note=document.createElement('p');note.textContent='Code repositories associated with this '+record.kind+'. Changes are saved in RobOS; the task-server issue is unchanged.';panel.append(title,note);
