...
function associateProject(dir,{id,projectId}){
 ...
 if(record.kind==='project')throw Error('Choose a feature or task.');
 let product=null;
 ...
 for(const r of records)if(ids.has(r.data.id)){r.data.product=product;r.data.updatedAt=Date.now();fs.writeFileSync(r.file,JSON.stringify(r.data,null,2));}
 return {ok:true};
}
