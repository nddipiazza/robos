...
# Story: Notifications App — Category-Filtered History Viewer
...
- Unread badge count displayed per category in the filter toolbar
...
- [x] Shows notifications in reverse chronological order with category icon and tier badge
- [x] Filter by category (multi-select), tier (multi-select), and date range
- [x] Full-text search across notification title and message
- [x] Per-category unread badge counts in sidebar
- [x] Total unread count exposed via IPC for system tray badge
- [x] Click-to-navigate opens relevant app context for PR, CI, task, and agent notifications
- [x] Bulk mark-as-read and clear-read actions work per category and globally
- [x] Preferences tab edits `~/.config/robos/notification-prefs.json` with live preview
- [x] Follows RobOS dark theme and conventions
- [x] Verified with automated E2E tests (`packages/robos-test/tests/notifications/e2e.test.js`) and persistent walkthrough archive in `~/.robos/development/walkthroughs/notifications/`.
